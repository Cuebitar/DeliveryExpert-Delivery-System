...
public class Delivery {
	...
	public String toString() {
		return String.format("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s", getDeliveryID(), getClient().getId(),
				getReceiver().getName(), getReceiver().getTelNo(), getStaff().getId(), isSameDayDelivery(),
				isWithInsurance(), getPickUpLocation(), getDropOffLocation(), getPickUpDate(), getItemList());
	}
	...
	protected void calculateAdditionalCharge() {
		additionalCharge = 0;
		if (sameDayDelivery) {
			additionalCharge += 10;
		}
